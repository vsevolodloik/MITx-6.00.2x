...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        been cleaned.
        """
        while self.room.isPositionInRoom(self.position.getNewPosition(self.direction, self.speed)) == False:
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for i in range(num_trials):
        ...
        # counter of time-steps in each trial        
        count = 0
        #initialize a room
        room = RectangularRoom(width, height)
        # initialize robots
        robots = [robot_type(room, speed) for n in range(num_robots)]
        while room.getNumCleanedTiles() / room.getNumTiles() < min_coverage:
            #anim.update(room, robots)            
            for r in robots:
                r.updatePositionAndClean()  
            #count up a time-step after all! robots have made one step   
            count += 1        
        steps.append(count)
        #anim.done()
...
'Aspect ratio = width/height', 'Number of steps')
