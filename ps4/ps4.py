...
def r_squared(y, estimated):
    ...
    """
    import numpy as np
    numerator = sum((np.array(y) - np.array(estimated))**2)
    denominator = sum((np.array(y) - np.mean(y))**2)
...
def evaluate_models_on_training(x, y, models):
    ...
    bestFit = 0
    for i in models:
        estimated = np.polyval(i, x)
        rSquared.append(r_squared(y, estimated))
    for i in range(0, len(rSquared)):
        if rSquared[i] == max(rSquared):
            bestFit = models[i]
    estimatedBestFit = np.polyval(bestFit, x)
    
    pylab.plot(x, y, 'bo', label = "True values")
    pylab.plot(x, estimatedBestFit, 'r-', label = "Estimated values")
    pylab.title("Degree: " + str(len(bestFit)-1)+ ", " + "R-squared: " + str(round(max(rSquared),3)))
...
for year in x1+x2:
    y.append(np.mean(raw_data.get_yearly_temp('BOSTON', year)))
models = generate_models(x1+x2, y, [1])    
evaluate_models_on_training(x1+x2, y, models)